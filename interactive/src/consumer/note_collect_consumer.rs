use crate::model::{
    CountBiz, NoteCollectMessage, NoteLikeMessage, NoteReadMessage, UserHistoryBiz,
};
...
pub struct NoteCollectConsumer {
    brokers: Vec<String>,
    interactive_repo: InteractiveRepo,
}

impl NoteCollectConsumer {
    ...
    pub fn consume(&self) -> anyhow::Result<()> {
        let mut con = Consumer::from_hosts(self.brokers.clone())
            .with_topic("NoteCollect".to_string())
        ...
        loop {
            ...
            for ms in mss.iter() {
                for m in ms.messages() {
                    let ret: serde_json::error::Result<NoteCollectMessage> =
            ...
            tokio::spawn(async move {
                handle_collect(interactive_repo, messages_batch_cloned).await;
            });
...
async fn handle_collect(interactive_repo: InteractiveRepo, messages: Vec<NoteCollectMessage>) {
