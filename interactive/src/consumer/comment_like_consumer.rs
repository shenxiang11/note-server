use crate::model::{
    CommentLikeMessage, CountBiz, NoteLikeMessage, NoteReadMessage, UserHistoryBiz,
};
...
pub struct CommentLikeConsumer {
    brokers: Vec<String>,
    interactive_repo: InteractiveRepo,
}

impl CommentLikeConsumer {
    ...
    pub fn consume(&self) -> anyhow::Result<()> {
        let mut con = Consumer::from_hosts(self.brokers.clone())
            .with_topic("CommentLike".to_string())
            .with_group("Comment".to_string())
        ...
        loop {
            ...
            for ms in mss.iter() {
                for m in ms.messages() {
                    let ret: serde_json::error::Result<CommentLikeMessage> =
...
async fn handle_count(interactive_repo: InteractiveRepo, messages: Vec<CommentLikeMessage>) {
    ...
    for (biz_id, count) in count_map.iter() {
        let ret = interactive_repo
            .save_count(CountBiz::CommentLike, *biz_id, *count)
            .await;
        if let Err(e) = ret {
