...
impl UserSrv {
    ...
    // 验证码有效期 10 分钟
    // 验证通过，或者验证三次失败，验证码失效
    pub async fn send_email_code(&self, email: &str) -> Result<String, AppError> {
    ...
    pub async fn signin(&self, email: &str, password: &str) -> Result<User, AppError> {
    ...
    pub async fn signup(&self, email: &str, password: &str, code: &str) -> Result<User, AppError> {
        let user = self.get_user_by_email(email).await;
        if user.is_ok() {
            return Err(AppError::DuplicateKey("email already exists".to_string()));
        }
